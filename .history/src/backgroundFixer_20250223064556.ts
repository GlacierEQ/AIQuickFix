...
export class BackgroundFixer {
    ...
    private async processQueue() {
        ...
        try {
            const apiKey = vscode.workspace.getConfiguration('haselerdev.aiquickfix').get<string>('apiKey');
            const model = vscode.workspace.getConfiguration('haselerdev.aiquickfix').get<string>('model') || 'open-source-model';
            ...
            const fix = await getGptFix(problem, problemCode, apiKey, model);

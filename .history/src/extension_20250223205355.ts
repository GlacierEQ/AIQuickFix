...
function openYourExtensionSettings() {
  vscode.commands.executeCommand("workbench.action.openSettings", "@ext:haselerdev.aiquickfix");
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand("haselerdev.aiquickfix.openSettings", () => {
  ...
  let disposable = vscode.commands.registerCommand("gptAIProblemFixerCommand", async (document: vscode.TextDocument, range: vscode.Range) => {
    console.log("running command");
    const problemString = document.getText(range);
    const extendedRange = extendRange(document, range, 10);
    const functionRange = await findEnclosingFunctionRange(document, range.start);
    const problemCode = document.getText(functionRange || extendedRange);

    const config = vscode.workspace.getConfiguration("haselerdev.aiquickfix");
    const apiKey = config.get<string>("apiKey");
    // Although 'model' setting exists, the open source service ignores it.
    if (!apiKey) {
      vscode.window.showErrorMessage("Please set your API key in settings", { modal: false }, { title: "Open Settings" })
        .then((selection: { title: string } | undefined) => {
          if (selection?.title === "Open Settings") {
    ...
    const fix = await getGptFix(problemString, problemCode);
...
function extendRange(document: vscode.TextDocument, originalRange: vscode.Range, n: number): vscode.Range {
...
async function findEnclosingFunctionRange(document: vscode.TextDocument, position: vscode.Position): Promise<vscode.Range | undefined> {
  ...
  function recursivelyFindEnclosingRange(symbols: vscode.DocumentSymbol[], position: vscode.Position): vscode.DocumentSymbol | undefined {
    for (const symbol of symbols) {
      if (symbol.children.length > 0) {
        const found = recursivelyFindEnclosingRange(symbol.children, position);
        if (found) { return found; }
      }
      if (symbol.range.contains(position)) { return symbol; }
    }
    return undefined;
  }
